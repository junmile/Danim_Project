...
<head>
    ...
    <script>
        ...
        var titlechk = /[가-힣]/;
        function titlecheck(){
           if($("input[name=helptitle]").val()==""){
              return true;
           }
           if(!titlechk.test($("input[name=helptitle]").val())){
              alert("제목을 확인해 주세요.");
              $("input[name=helptitle]").val("");
        ...
        function concheck(){
           ...
           if(!conchk.test($("textarea[name=contents]").val())){
   ...
   <!-- /Header -->
    ...
    <div id="information" class="section">
        ...
        <div class="container">
            ...
            <br> <br>
            <!-- row -->
            <form action="insertdanim.do" method="POST" enctype="multipart/form-data" name="insertForm">
                <div class="row">
                    <div class="col-md-6 row-md-6 ">

                        <table class="myinfo-table insertinfoyj" cellpadding="30px" style="margin-bottom:20px;">
                            <tr class="">
                                <select name="category" class="selectyj" id="field" style="margin-bottom:20px;" required>
                    ...
                    <div class="col-md-12 row-md-6 mapyj" style="padding-bottom: 40px;">
                        <table cellpadding="30px" class="col-md-4 col-sm-4 col-lg-6">
                            <input type="address" name="detailaddress" id="addressdetailyj" placeholder="상세주소" required>
                        </table>
                    </div>
                    <div class="col-md-12">
                        <table class="myinfo-table " cellpadding="30px" style="margin-bottom: 40px;">
                            <tr class="tryj fullwidthyj" style="letter-spacing: inherit;" id="timevalyj">
                                <td class="col-md-3 reser" style="width:15%;">
                                    <select name="endtime" class="boxstyleyj" id="timeryj" style="" required>
                                ...
                                <td class="col-md-3 reser" style="padding: 0px;" id=""><select name="resertime" class="boxstyleyj" id="reser2" style="display: none;" hidden="false">
                                        ...
                                        <option value="" disabled selected hidden>예약시각</option>
                                        <option value="00:00">00시</option>
                    ...
                    <div class="col-md-12 row-md-6 mapyj" style="padding-bottom: 40px;">
                        <table cellpadding="30px" class="col-md-4 col-sm-4 col-lg-6">
                            <input type="text" name="helptitle" id="helptitleyj" placeholder="제목을 적어주세요 ex)가구옮기는것을 도와주세요!!" onblur="titlecheck();" required>
                        </table>
                    </div>
                    
                    <div class="col-md-12 tryj" style="margin-bottom: 40px;">
                        <textarea name="contents" placeholder="집 안 가구위치 변경하려 합니다. 아파트 34평형 구조입니다." style="width: 100%; height: 200px;" onblur="concheck();" required></textarea>
                    </div>
                ...
                <div class="col-md-12 row-md-6 mapyj" style="padding-bottom: 100px;">
                        <table cellpadding="30px" class="col-md-4 col-sm-4 col-lg-6">
                            <input class="" type="text" name="pay" id="inputmoney" placeholder="심부름비용(물품구매비용 제외)" onkeyup="inputNumberFormat(this)"  required>
                        </table>
                    </div>
                    ...
                    <br> <br>
                    <!-- /row -->
                    <a id="buttonkey" class="main-button icon-button pull-right enrollbutton" style="cursor:pointer;" onclick="moneycheck();">작성완료</a>
                </div>
            </form>
            <!-- /container -->
...
<input type="text" id="money" value="${money}" hidden="true">
    <!-- map -->
...
function moneycheck(){ //버튼 클릭시 radio값 가져
   var radioVal = $('input[name="paym"]:checked').val();
   var inputmoney = $('#inputmoney').val();
      var imoney = inputmoney.replace(/,/g, '');
   var vmoney = Number(imoney);
   var sub = Number(money) - Number(imoney);
   var umoney = String(imoney);
   var division = '진행중';
      
      
      if(radioVal == "코인결제"){ //radio 값이 코인결제일 경우
            //넘었다면 실행
             if(vmoney < 5000){
            //5000원 안넘었다면 실행
               alert("심부름값 최소금액은 5,000원 입니다.\n 다시 입력해주세요.");
               $('#inputmoney').val('');
            }else if(vmoney > money){
            //소지금 보다 초과시
               var result = confirm("심부름값이 소지금 보다 많습니다.\n" + "결제페이지로 이동하시겠습니까?");
               if(result){
                  /* alert("다님코인 충전소로 이동합니다."); */
            ...
            }else if (confirm("다님코인으로 선택하셨습니다. 정말로 작성하시겠습니까?\n(작성하신 내용은 '서비스 현황'에서 수정이 가능합니다.)")){
               alert("다님코인으로 진행됩니다.\n");
               document.insertForm.submit();
            } //if(vmoney >= 5000 && vmoney > money)
            
      }else{ //if(radioVal == "현장결제"){}
      //현장결제로 접근했을 때      
         if(vmoney < 5000){
            alert("심부름값 최소금액은 5,000원 입니다.\n" + "다시 입력해주세요.");
            $('#inputmoney').val('');
         }else if(confirm("현장결제로 선택하셨습니다. 정말로 작성하시겠습니까?\n(작성하신 내용은 '서비스 현황'에서 수정이 가능합니다.)")){ //if(vmoney >= 5000){}
            alert("현장결제로 진행됩니다.\n");
            document.insertForm.submit();
         }
      }
