...
.close {position: absolute;top: 10px;right: 10px;color: #white;width: 17px;height: 17px;
	...
	$.ajax({
		...
		success:function(sendjson){
			...
			if(sendjson.page.listcount!=0){
				var searchcate= decodeURIComponent(sendjson.searchcate).replace(/\+/gi, " ");
				var searchname= decodeURIComponent(sendjson.searchname).replace(/\+/gi, " ");
	         ...
	         <!-- /row -->
				...
				$('#main1').html("<div class='nosearch row' style='margin-top:20%;float:left;width:100%; text-align:center;'><img src='/danim/resources/img/nosearch3.png'class='' style='margin-bottom:50px; width:20%;text-align:center;'><p class='imgtext'>검색된 결과가 없습니다.</p></div>");
				$('#main2').html("");
			}
		}		
	})
...
<body>
		...
		<div class="container">
			...
			<div class="row">
				...
				<div id="aside" class="col-md-3">
					...
					<!-- tags widget -->
					<div class="widget tags-widget" id="tuijiantag">
						<h3>추천 검색 태그</h3>
	...
	<script>
		...
		var iwContent = '<div style="padding:5px; background-color:white;">Hello World!</div>', // 인포윈도우에 표출될 내용으로 HTML 문자열이나 document element가 가능합니다
		...
		var cleaningWin=[], deliveryWin=[], shoppingWin=[], keepingWin=[], lineupWin=[], careWin=[], cosplayWin=[], professionalWin=[], movingWin=[];
		
		function ready() {
			<c:forEach items="${helplist}" var="help" >
			create("${help.category}", "${help.helptitle}",
					"${help.detailaddress}", Number("${help.locationcodelat}"),
					Number("${help.locationcodelng}"), "${help.pay}", markerImageSrc, "${help.helpno}");
			</c:forEach>
		};

		function create(category, title, add, lat, lng, pay, src, helpno) {
			var ward = new Object;
			ward.title = title;
			...
			if (category == '청소해주기') {
				...
				cosplayWin.push(ward);
			} else if (category == '전문가/비지니스') {
				professionalPositions.push(new kakao.maps.LatLng(lat, lng));
				professionalWin.push(ward);
		...
		function createMarker(position, image, place) {
			
			/* console.log("createMarker.position : " + position);
			console.log("createMarker.image.src : " + JSON.stringify(image)); */
			// 'position: absolute; z-index: 0; white-space: nowrap; margin: -65px 0px 0px -109px; left: 590px; top: 106px;'
			var content = "<div style='position: absolute; z-index: 0; white-space: nowrap; margin: -165px 0px 0px -120px;'><div class='placeinfo'>"
            + "<div class='title' style='width:250px;'><a class='titlesec' href='helpdetail.do?helpno="+Number(place.helpno)+"'>" + place.title + "</a>"
            + "<div class='close' onclick='closeOverlay()' style='margin:3px; width:17px;' title='닫기'></div></div>"
            + "<span style='clear:both;'>"+place.add+"</span>"
            + "<span style=''>"+place.category+"</span><span style=''>￦"+place.pay+"원</span>"
            + "</div>"
            + "<div class='after'></div></div>";
			
			var marker2 = new kakao.maps.Marker({position : position, image : image});
			...
			addEventHandle(content, 'mousedown', kakao.maps.event.preventMap);
			addEventHandle(content, 'touchstart', kakao.maps.event.preventMap);
			 */
			return marker2;
		}
		...
		function createCleaningMarkers() {
			...
			for (var i = 0; i < cleaningPositions.length; i++) {
				...
				, marker = createMarker(cleaningPositions[i], markerImage, cleaningWin[i]);
		...
		function createDeliveryMarkers() {
			for (var i = 0; i < deliveryPositions.length; i++) {
				...
				var markerImage = createMarkerImage(markerImageSrc, imageSize,
						...
						deliveryPositions[i], markerImage, deliveryWin[i]);

				// 생성된 마커를 물건배달 마커 배열에 추가합니다
				delivery.push(marker);
			}
			// document.getElementsByClassName('placeinfo').parent().css('margin', '-65px 0px 0px -109px');
		...
		function createShoppingMarkers() {
			for (var i = 0; i < shoppingPositions.length; i++) {
				...
				var markerImage = createMarkerImage(markerImageSrc, imageSize,
						imageOptions), marker = createMarker(
						shoppingPositions[i], markerImage, shoppingWin[i]);
		...
		function createKeepingMarkers() {
			for (var i = 0; i < keepingPositions.length; i++) {
				...
				var markerImage = createMarkerImage(markerImageSrc, imageSize,
						imageOptions), marker = createMarker(
						keepingPositions[i], markerImage, keepingWin[i]);
		...
		function createLineupMarkers() {
			for (var i = 0; i < lineupPositions.length; i++) {
				...
				var markerImage = createMarkerImage(markerImageSrc, imageSize,
						imageOptions), marker = createMarker(
						lineupPositions[i], markerImage, lineupWin[i]);
		...
		function createCosplayMarkers() {
			for (var i = 0; i < cosplayPositions.length; i++) {
				...
				var markerImage = createMarkerImage(markerImageSrc, imageSize,
						imageOptions), marker = createMarker(
						cosplayPositions[i], markerImage, cosplayWin[i]);
		...
		function createProfessionalMarkers() {
			for (var i = 0; i < professionalPositions.length; i++) {
				...
				var markerImage = createMarkerImage(markerImageSrc, imageSize,
						imageOptions), marker = createMarker(
						professionalPositions[i], markerImage, professionalWin[i]);
		...
		function createMovingMarkers() {
			for (var i = 0; i < movingPositions.length; i++) {
				...
				var markerImage = createMarkerImage(markerImageSrc, imageSize,
						imageOptions), marker = createMarker(
						movingPositions[i], markerImage, movingWin[i]);
